...
#             Range, "/my_robot/range/left", self.left_range_callback, 10)
#         self.center_range_subscriber_ = self.create_subscription(
#             Range, "/my_robot/range/center", self.center_range_callback, 10)
#         self.right_range_subscriber_ = self.create_subscription(
#             Range, "/my_robot/range/right", self.right_range_callback, 10)
...
#         self.range_dict = {
#             "left_ultrasonic_frame": [],
#             "center_ultrasonic_frame": [],
#             "right_ultrasonic_frame": []
#         }

#         # Initialise a dictionary to store the latest range readings
#         self.latest_ranges = {
#             "left_ultrasonic_frame": float('inf'),
#             "center_ultrasonic_frame": float('inf'),
#             "right_ultrasonic_frame": float('inf')
#         }
...
#         self.process_range(msg, "left_ultrasonic_frame", self.left_laserscan_publisher)

#     def center_range_callback(self, msg: Range):
#         self.process_range(msg, "center_ultrasonic_frame", self.center_laserscan_publisher)

#     def right_range_callback(self, msg: Range):
#         self.process_range(msg, "right_ultrasonic_frame", self.right_laserscan_publisher)
...
#                     f"Left: {self.latest_ranges['left_ultrasonic_frame'] * 100:.2f} cm, " +
#                     f"Center: {self.latest_ranges['center_ultrasonic_frame'] * 100:.2f} cm, " +
#                     f"Right: {self.latest_ranges['right_ultrasonic_frame'] * 100:.2f} cm"
#                 )
...
#         scan_msg.header.frame_id = frame_id  # Use the same frame_id as the Range message
...
class UltrasonicToLaserScanNode(Node):
    ...
    def range_to_laserscan(self, frame_id: str) -> LaserScan:
        scan_msg = LaserScan()
        scan_msg.header.stamp = self.get_clock().now().to_msg()  # Set timestamp
        scan_msg.header.frame_id = "base_ultrasonic_frame"  # Use a common frame_id for all LaserScan messages
        scan_msg.angle_min = -np.pi  # -180 degrees in radians
        scan_msg.angle_max = np.pi   # 180 degrees in radians
        scan_msg.angle_increment = np.pi / 180  # 1 degree resolution in radians
        scan_msg.time_increment = 0.0  # Time between measurements [s]
